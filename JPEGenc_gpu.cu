...
void trans_from_host_to_device(
	int n,int m,
	UC *graph,
	float *T, float *bias, char *zigzag,
	char *lq, char *cq
) {
	...
	CHECK(cudaMemset(resCb_d,-1,sizeof(int)*fullsize));
	CHECK(cudaMemset(resCr_d,-1,sizeof(int)*fullsize));
}
...
void trans_from_device_to_host() {
	...
	CHECK(cudaMemcpy(resCb,resCb_d,sizeof(int)*fullsize,cudaMemcpyDeviceToHost));
	CHECK(cudaMemcpy(resCr,resCr_d,sizeof(int)*fullsize,cudaMemcpyDeviceToHost));
	CHECK(cudaFree(G_d));
	CHECK(cudaFree(T_d));
...
__global__ void kernel(
	...
	int id=mx*m+my;
	int yid=((tx>>3)<<1)|(ty>>3);
	int blockLid=blockIdx.y*(((m+15)>>4)<<4)+blockIdx.x;
	
	__shared__ float Y[MCUSIZE][MCUSIZE],Cb[BLOCKSIZE][BLOCKSIZE],Cr[BLOCKSIZE][BLOCKSIZE];
	...
	id=tx*MCUSIZE+ty;
	scan(MCUSIZE);
	if ((id&63)==0 || F[id]!=0) resY_d[(blockLid<<8)|flag[id]]=(F[id]<<7)|bl;

	// Cb DCT and quantize
	DCT_and_quantize(Cb,cq_d);
	// Cb run-length coding
	scan(BLOCKSIZE);
	if (id<64 && (id==0 || F[id]!=0)) resCb_d[(blockLid<<6)|flag[id]]=(F[id]<<7)|bl;
	
	// Cr DCT and quantize
	DCT_and_quantize(Cr,cq_d);
	// Cr run-length coding
	scan(BLOCKSIZE);
	if (id<64 && (id==0 || F[id]!=0)) resCr_d[(blockLid<<6)|flag[id]]=(F[id]<<7)|bl;
}

void encode_block(int *res,int st,int en,const int *DC_T, const int *AC_T,JPEGencoder *ts) {
	int lastPos=63;
	for(int j=st; j<en; j++) {
		if (res[j]&(1<<6)) break;
		...
		if (pos==0) {	// Y DC
			...
			ts->huffman_coding(0,val-lastYDC,DC_T);
			lastYDC=val;
		} else {		// Y AC
...
float JPEGencoder::encode_gpu() {
	...
	grid=dim3((m+15)>>4,(n+15)>>4,1);
	block=dim3(16,16,1);
	kernel_cntsm<<<grid,block>>>(n,m,R_d,G_d,B_d,T_d,bias_d,zigzag_d,resY_d,resCb_d,resCr_d);
	...
	for(int i=0; i<MCUnum; i++) {
		encode_block(resY,(i<<8),((i+1)<<8),Y_DC_T,Y_AC_T,this);
		encode_block(resCb,(i<<6),((i+1)<<6),C_DC_T,C_AC_T,this);
		encode_block(resCr,(i<<6),((i+1)<<6),C_DC_T,C_AC_T,this);
	}
